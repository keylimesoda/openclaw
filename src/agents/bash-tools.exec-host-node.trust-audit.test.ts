...
describe("node-host trust audit failures", () => {
  beforeEach(() => {
    callGatewayTool.mockReset();
    appendTrustAuditEntry.mockReset();
    appendTrustAuditEntry.mockImplementation(() => {
      throw new Error("audit-write-failed");
    });
  });

  it("preserves original node invoke error when audit append fails in catch path", async () => {
    ...
    expect(appendTrustAuditEntry).toHaveBeenCalledTimes(1);
  });

  it("does not fail successful node exec when audit append throws in success path", async () => {
    callGatewayTool.mockImplementation(async (method: string, _opts: unknown, params: unknown) => {
      if (method !== "node.invoke") {
        return { ok: true };
      }
